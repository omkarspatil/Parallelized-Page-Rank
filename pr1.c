	...
	if(world_rank == 0)
	{
		...
		do
		{
			//printf("---------------------------------------------------Iteration %d---------------------------------------------\n",iterations);
					...
					prank_send_buffer = (double*) malloc (sizeof(double)*n*world_size);

					for(int i = 0; i< world_size; i++)
					{
						memcpy(prank_send_buffer + i*n, prank_part, sizeof(double)*n);
					}
					...
					MPI_Alltoallv(prank_send_buffer,send_counts,send_displs,MPI_DOUBLE,prank_full,recv_c,recv_d,MPI_DOUBLE,MPI_COMM_WORLD);
